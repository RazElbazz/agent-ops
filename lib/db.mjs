...
// DB path is env-configurable (AGENT_OPS_DB) so tests/demos can use a scratch DB and you can run
// multiple isolated instances. Defaults to a local, gitignored data.db.
export const DB_PATH = process.env.AGENT_OPS_DB || join(ROOT, 'data.db')
...
CREATE TABLE IF NOT EXISTS operations (
  ...
  deps TEXT DEFAULT '[]', uses TEXT DEFAULT '[]', version INTEGER DEFAULT 1, updated_at TEXT
);
...
CREATE TABLE IF NOT EXISTS traces (
  ...
  input TEXT, output TEXT, status TEXT, note TEXT, ms INTEGER
);
-- sessions: the live coordination layer ("pin board"). Each parallel chat/agent reports what it is
-- working on right now, so every chat can see who is doing what and no two duplicate each other.
CREATE TABLE IF NOT EXISTS sessions (
  chat TEXT PRIMARY KEY, title TEXT, detail TEXT, op TEXT, chain TEXT DEFAULT '[]',
  status TEXT DEFAULT 'active', started_at TEXT, updated_at TEXT
);
...
CREATE INDEX IF NOT EXISTS idx_actions_chat ON actions_log(chat);
`)

// Lightweight migrations so existing databases gain new columns (ALTER throws harmlessly if the
// column already exists — the CREATE above covers fresh databases).
for (const alter of [
  "ALTER TABLE operations ADD COLUMN uses TEXT DEFAULT '[]'",
  'ALTER TABLE traces ADD COLUMN ms INTEGER',
